...
import DevCards from './Dev/DevCards'
...
const Development = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5055/development').then((res) => {
            if (res.ok) {
                return res.json()
            }
        }).then((jsonRes) => {
    ...
    return (
        <>
            <div className="transition-all md:mx-12 mx-7 bg-white rounded-b-xl h-auto pb-4 mb-10">
                <div className="flex text-left flex-col sm:flex-row sm:justify-between lg:justify-between sm:items-center py-8 md:px-8 px-4">
                    <div className="flex items-center">
                        ...
                        }} className="transition-all rounded-full bg-gray-800 w-6 h-6 md:w-12 md:h-12 flex items-center justify-center cursor-pointer focus:bg-gray-700 text-white font-bold text-xl hover:w-8">+</div>
                        <h2 className="ml-3 md:ml-8 text-xl md:text-2xl font-bold">Devloping cards</h2>
                    </div>
            ...
            <DevForm showForm={showForm} setShowForm={setShowForm} />
        </>
    )
}
