...
import DoneForm from './DoneCom/DoneForm'
import DoneCards from './DoneCom/DoneCards'
...
const Done = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5055/done').then(res => {
            if (res.ok) {
                return res.json()
            }
    ...
    return (
        <>
            <div className="transition-all md:mx-12 mx-7 bg-white rounded-b-xl h-auto pb-4 mb-10">
                <div className="flex text-left flex-col sm:flex-row sm:justify-between lg:justify-between sm:items-center py-8 md:px-8 px-4">
                    <div className="flex items-center">
                        ...
                        <h2 className="ml-3 md:ml-8 text-xl md:text-2xl font-bold">Done cards</h2>
                    </div>
            ...
            <DoneForm showForm={showForm} setShowForm={setShowForm} />
        </>
    )
}
